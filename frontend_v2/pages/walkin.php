...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama === '' || $jumlah < 1) {
        ...
        if ($result['ok']) {
            $message = "Tamu '$nama' ditambahkan ke Waiting List. Anda akan diberi tahu bila meja tersedia.";
            $messageType = 'success';
        } else {
            // Fallback demo bila backend tidak tersedia
            $message = "Tidak ada meja kosong untuk $jumlah orang di area $area. Tamu '$nama' ditambahkan ke Waiting List (demo).";
            $messageType = 'info';
        }
    }
}
...
if (empty($waitingList)) {
    $waitingList = [
        ['name' => 'Bayu Saputra', 'number_of_guest' => 4, 'area' => 'outdoor', 'created_at' => date('Y-m-d') . ' 10:15:00', 'status' => 'waiting'],
        ['name' => 'Indah Permata', 'number_of_guest' => 6, 'area' => 'vip', 'created_at' => date('Y-m-d') . ' 10:30:00', 'status' => 'waiting'],
        ['name' => 'Fajar Nugroho', 'number_of_guest' => 2, 'area' => 'indoor', 'created_at' => date('Y-m-d') . ' 10:45:00', 'status' => 'waiting'],
    ];
}
?>
