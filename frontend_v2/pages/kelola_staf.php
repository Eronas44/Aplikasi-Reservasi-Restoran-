...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama === '' || $email === '' || $password === '') {
        ...
        if ($result['ok']) {
            $message = "Akun staf '$nama' ($email, role: $roleStaff) berhasil dibuat.";
        } else {
...
if (isset($_GET['toggle']) && is_numeric($_GET['toggle'])) {
    ...
    if ($target['ok'] && isset($target['data']['data'])) {
        ...
        if ($result['ok']) {
            ...
            $message = api_error_message($result, 'Gagal memperbarui akun.');
            $messageType = 'red';
        }
    }
}

// Ambil daftar user dari backend, filter hanya role staff
$stafList = [];
$usersResult = api_get(API_USERS . '?limit=100');
if ($usersResult['ok']) {
    $raw = $usersResult['data']['data'] ?? [];
    foreach ($raw as $u) {
...
if (empty($stafList)) {
    $stafList = [
        ['user_id' => 0, 'name' => 'Front Staff', 'email' => 'staff@reservasi.local', 'role' => 'staff'],
        ['user_id' => 0, 'name' => 'Resepsionis A', 'email' => 'resepsionis@resto.local', 'role' => 'staff'],
        ['user_id' => 0, 'name' => 'Resepsionis B', 'email' => 'resepsionis2@resto.local', 'role' => 'staff'],
        ['user_id' => 0, 'name' => 'Kasir', 'email' => 'kasir@resto.local', 'role' => 'staff'],
    ];
}
?>
